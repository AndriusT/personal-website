...
$$S = \int \mathrm{d}x^{4}\sqrt{-g}\bigg(\frac{M*{Pl}^{2}}{2} R - \frac{1}{2} \phi^{,\mu} \phi*{,\mu} - V(\phi) - \frac{\mathcal{L}*{m}\left(\varphi*{m}^{(i)}, \tilde{g}_{\mu \nu}^{(i)}\right)}{\sqrt{-g}} \bigg),$$
...
$$
\square \phi=V*{, \phi}-\sum*{i} \frac{\beta*{i}}{M*{p l}} T*{\mu \nu}^{(i)} \tilde{g}*{(i)}^{\mu \nu}
$$